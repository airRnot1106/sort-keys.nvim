local normalize = require("sort-keys.parse.languages.python.normalize")
...
describe("languages.python.normalize", function()
  ...
  it("strips the delimiter off a triple-quoted key", function()
    assert.are.equal("triple", normalize('"""triple"""'))
    assert.are.equal("triple", normalize("'''triple'''"))
  end)
end)
